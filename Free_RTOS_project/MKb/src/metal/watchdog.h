...
enum metal_watchdog_result {
    ...
    METAL_WATCHDOG_INTERRUPT, /*!< When the watchdog triggers, fire an interrupt,
                                  but never trigger interrupt. */
    METAL_WATCHDOG_FULL_RESET, /*!< When the watchdog triggers, cause a full
                                  system reset if previous interrupt is not cleared. */
    METAL_WATCHDOG_FULL_RESET_NMI, /*!< When the watchdog triggers, cause a full
                                  system reset if previous interrupt is not cleared, and WDGINT is 
                                  connected with NMI. */
};
...
/*!
 * @brief Get the watchdog timer value reigster.
 *
...
/*!
 * @brief Not needed in current implementation
 *
 * @return 0.
 */
