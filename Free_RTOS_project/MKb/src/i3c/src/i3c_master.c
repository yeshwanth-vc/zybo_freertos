...
int32_t i3c_master_init()
{
	...
	//m_cpu = metal_cpu_get(metal_cpu_get_current_hartid());
	if (m_cpu == NULL) {
	    return 1;
	}
//	m_cpu_intr = metal_cpu_interrupt_controller(m_cpu);
	if (m_cpu_intr == NULL) {
	     //printf("Cpu interrupt controller failed");
	     return 3;
	}
//	metal_interrupt_init(m_cpu_intr);
	
//	mInterrupt = metal_interrupt_get_controller(2,metal_cpu_get_current_hartid());
	if(mInterrupt == NULL) {
		//printf("Get interrupt controller failed");
	}
/*
	metal_interrupt_init(mInterrupt);
	if(metal_interrupt_set_vector_mode(mInterrupt, 3) != 0) {
		//printf("Set interrupt vector mode failed");
		return 1;
	}
*/
	/*
	if(metal_interrupt_register_vector_handler(mInterrupt,I3C_INTERRUPT_ID,&i3c_master_interrupt_handler,NULL) != 0) {
...
/*
	if(metal_interrupt_enable(mInterrupt,I3C_INTERRUPT_ID) != 0) {
		//printf("Set interrupt vector enable failed");
		return 1;
	}
	if (metal_interrupt_enable(m_cpu_intr, 0) == -1) {
