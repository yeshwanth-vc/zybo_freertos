...
//ddr4 main calls 
	//xil_print("ddr4 calls are running\r\n");
#if SPD_ENABLE
	spd_i2c_read();
#endif

	printf("ddr4 calls gfh_mc_init  init called from mian\r\n");
	gfh_mc_init();
	printf("ddr4 calls ddrphy init called\r\n");
	ddr_phy_init();

	gfh_mc_post_init();

	printf("goldfinch main is ended\r\n");
	while(1);
 
 
 
    printf("goldfinch d2d init\r\n");
  uint8_t err;
  debug_gpio_set(0x7);
#ifdef I3C_EN
	err = i3c_slave_init();
...
#ifdef D2D_EN
    printf("d2d_mode_init is called fom gfh main\r\n");
...
*/
printf("d2d init is completed from main in gfh\r\n");
printf(".............\r\n");
}
