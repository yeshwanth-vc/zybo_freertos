...
void jtag_send_dr_data(uint8_t apb_command[9], uint32_t *tms) {
    uint64_t dr_data_low = 0;
    uint8_t dr_data_high = apb_command[8]; // Last byte (pwrite, pprot, etc.)
    
    memcpy(&dr_data_low, apb_command, 8);
    printf("JTAG Shift DR Data (72-bit): 0x%02X%016llX\n", dr_data_high,dr_data_low);
...
void jtag_sequence(uint8_t apb_command[9]) {
...
void register_write(uint32_t addr, uint32_t data, uint32_t read_write) {
    ...
    apb_data_low |= ((uint64_t)apb_data.paddr   & 0xFFFFFFFF) << 34; // Bits [63:34] - Address
    uint8_t apb_data_high = ((apb_data.paddr >> 30) & 0x3);
    
    // Store first 64 bits in `apb_command`
