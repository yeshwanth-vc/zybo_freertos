...
/*
// Below code add upto 2399 line or endif 
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 0, 0)
	   		 if (priv->netdev->ieee80211_ptr && priv->netdev->ieee80211_ptr->use_4addr && priv->netdev->ieee80211_ptr->iftype == NL80211_IFTYPE_STATION) {
				t_u32 transaction_id;
				t_u32 hash_key;
				transaction_id =
					woal_get_dhcp_discover_transation_id(
...
#else
		    if (priv->wdev && priv->wdev->use_4addr && priv->wdev->iftype == NL80211_IFTYPE_STATION) {
				t_u32 transaction_id;
				t_u32 hash_key;
				transaction_id =
					woal_get_dhcp_discover_transation_id(
...
#ifdef CONFIG_ARCH_IMX                              //added 4 lines
	if (IS_PCIE(pmhandle->card_type)) {
		release_bus_freq(BUS_FREQ_HIGH);
	}
#endif
