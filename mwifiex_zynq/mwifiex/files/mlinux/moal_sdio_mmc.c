...
{
	PRINTM(MMSG, "woal_sdiommc_bus_register...\n");	
	mlan_status ret = MLAN_STATUS_SUCCESS;

	ENTER();
		
	int i;
       for (i = 0; wlan_ids[i].vendor != 0; i++) {
               printk(KERN_INFO, "Supported SDIO device: vendor=0x%x device=0x%x\n",wlan_ids[i].vendor, wlan_ids[i].device);
        }	
...
/*Added code */
	...
	for (retry = 0; retry < 30; retry++) {
   		 woal_sdiommc_read_reg(handle, handle->card_info->magic_reg, &magic);
  		  magic &= 0xFF;

   		 if (magic == CHIP_MAGIC_VALUE) {
     		   PRINTM(MMSG, "Magic value OK: 0x%x (retry %d)\n", magic, retry);
     		   break;  // Success
   		 }

  		  PRINTM(MMSG, "Invalid magic 0x%x (retry %d), waiting...\n", magic, retry);
   		 mdelay(10);  // Wait 10ms before next retry
}
/*==================*/
	/** Revision ID register */
	woal_sdiommc_read_reg(handle, host_strap_reg, &strap);

    	//PRINTM(MMSG, "magic = 0x%x\n", magic);
	strap &= 0x1;
	//magic &= 0xFF;
	/* 1 = SDSD, 0 --SD UART */
